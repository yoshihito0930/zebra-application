...
# ========================================
# 埋め込みウィジェット配信用 S3（zebra-widget.js）
# SPA用バケットとは分離する。SPA の「404→index.html」フォールバックが
# ウィジェットJSの取得に干渉しないよう、別オリジン + /widget/* ビヘイビアで配信する。
# ========================================
resource "aws_s3_bucket" "widget" {
  bucket = "${var.environment}-zebra-widget"
...
resource "aws_cloudfront_origin_access_identity" "widget" {
  comment = "${var.environment} Zebra Widget OAI"
}
    ...
    Statement = [
      {
        Effect = "Allow"
        Principal = {
          AWS = aws_cloudfront_origin_access_identity.widget.iam_arn
        }
        Action   = "s3:GetObject"
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  # 埋め込みウィジェット（/widget/*）専用ビヘイビア
  # 別オリジン（widgetバケット）へルーティングし、SPAの404フォールバックを受けない。
  # JS更新を速く反映させるため TTL は短めに設定。
