...
| ロール | 説明 |
|--------|------|
| customer | 会員。自分の予約・問い合わせのみ操作可能。**フロントエンドの利用者向け導線では未使用**（会員/ゲスト区分の撤廃により公開予約へ一本化、2026-06-21）。admin 管理画面からの会員予約パスとバックエンド実装に温存 |
| guest | 公開予約の利用者。認証は不要で、予約時に発行されるトークンで予約の確認・キャンセル・昇格を行う（ロールというより認証方式。`POST /reservations/guest` 系で利用） |
| admin | スタジオ管理者。所属スタジオの全データを操作可能 |
| staff | スタジオスタッフ。所属スタジオの予約データを閲覧のみ可能 |

> フロントエンドの利用者向け導線は会員/ゲスト区分を撤廃し公開予約（ログイン不要）に一本化済み（2026-06-21）。本セクションの `customer`/`guest` ロール・ゲスト予約エンドポイント・`is_guest` フラグは、その公開予約の実装基盤としてバックエンドに残存しているもの。経緯と現状の対応関係は [guest-user-implementation.md](guest-user-implementation.md) を参照。
...
| バッファタイムチェック | 既存予約の前後1時間バッファゾーンに対する可否を判定（下記） |

##### バッファタイム（前後1時間）の可用性判定

既存予約ごとに前後1時間のバッファゾーン `[開始-1h, 終了+1h)` を生成し、申込時間帯との重なりを評価する。区間はすべて半開区間 `[start, end)`、重なり判定は `(a < d かつ c < b)` で行う（境界が接するだけのケースは重ならない＝可。例: 既存 10:00-12:00 のちょうど 1 時間後 13:00 開始は可）。

| 既存予約の状態 | バッファゾーンに重なった場合の結論 |
|--------------|--------------------------------|
| confirmed（本予約） | **予約不可**（`BUFFER_TIME_CONFLICT`） |
| tentative（仮予約） | **第2キープのみ可**（`SECOND_KEEP_ONLY`） |

- 複数ゾーンに重なる場合は厳しい順に採用する: 予約不可 > 第2キープのみ可 > 通常（本予約ゾーンと仮予約ゾーンが重複する区間は予約不可が優先）。
- 申込種別ごとの扱い:
  - **本予約・仮予約**: 結論が「予約不可」なら `BUFFER_TIME_CONFLICT`、「第2キープのみ可」なら `SECOND_KEEP_ONLY` を返す（希望種別に関わらず、第2キープへの切替を促す）。
  - **第2キープ**: バッファゾーンに重なっても作成可（前提条件は上記「第2キープの前提」で別途検証）。
  - **ロケハン**: バッファ制約を受けない（常に評価対象外）。
...
| BUFFER_TIME_CONFLICT | 指定の時間帯は既存予約の前後59分以内です。第2キープまたはロケハンをご利用ください | 既存「本予約」のバッファゾーン内に本予約/仮予約を作成しようとした |
| SECOND_KEEP_ONLY | 指定の時間帯は第2キープのみ予約可能です | 既存「仮予約」のバッファゾーン内に本予約/仮予約を作成しようとした（第2キープなら作成可） |
