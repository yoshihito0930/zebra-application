...
- User (会員、スタジオ管理者、スタジオスタッフ)

> 注: フロントエンドの利用者向け導線は会員/ゲスト区分を撤廃し公開予約（ログイン不要）に一本化済み（2026-06-21）。公開予約の利用者は User エンティティ（user_id）を持たず、トークンで予約を確認する。本ドキュメントの `customer`/`guest` ロールや `is_guest`・トークン認証に関わるデータ構造は、その公開予約の実装基盤としてバックエンドに残存しているもの。詳細は [guest-user-implementation.md](guest-user-implementation.md) を参照。
- Studio (スタジオ)
- Reservation (予約)
- Plan (料金プラン)
- BlockedSlot (予約不可枠)
...
### スタジオ利用者（公開予約・会員予約の実装基盤）

> 以下のアクセスパターンは、会員/ゲスト区分の撤廃前に設計したもの。現状フロントエンドの利用者向け導線は公開予約（ログイン不要）に一本化しており、AP-01/02（ユーザ登録）・AP-09/15（自分の予約・問い合わせ一覧を user_id で取得）は会員予約パス（admin 管理画面・バックエンドに温存）でのみ使用する。公開予約の利用者は、予約時に発行されるトークンで AP-10（予約IDで1件取得）相当の確認を行う。
