...
##### GSI2: スタジオ別ロール検索（2026-05-18追加）
| キー | 値 | 説明 |
|------|-----|------|
| PK | studio_id | スタジオID |
| SK | role | ロール（customer / admin / staff） |

**用途**: 予約通知の管理者宛先取得など、スタジオに所属する admin / staff ユーザー一覧を取得する用途
**注意**: customer は studio_id を持たないため、このインデックスには出現しない
...
| AP-51 | スタジオの管理者一覧取得 | GSI2 | PK=studio_id, SK=role (="admin") |
