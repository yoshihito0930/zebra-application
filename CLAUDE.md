...
**ユーザー**:
- **公開予約利用者**: ログイン不要でカレンダーを閲覧し、予約できる一般利用者。会員/ゲストの区分は撤廃し、公開予約導線に一本化（2026-06-21）
- **スタジオ管理者 (admin)**: 予約の承認・管理を行うスタジオ運営者
- **スタジオスタッフ (staff)**: 予約の閲覧のみ可能なスタッフ

> 注: バックエンド/API/DynamoDB 上には `customer`・`guest` ロールやゲスト予約API・`is_guest` フィールド・トークン認証が実装として残存している（無変更）。フロントエンドはこれらをそのまま「公開予約の実装基盤」として利用しつつ、UI 上の会員/ゲスト区分を撤廃して公開予約導線に一本化している。

**主要機能**:
- 予約カレンダー表示・予約作成（本予約/仮予約/ロケハン/第2キープ）
- **公開予約機能**（ログイン不要、トークンベースで予約確認・キャンセル）（2026-06-21 公開予約導線へ一本化）
...
- **メール通知機能**（SES統合、予約確認・キャンセル・昇格通知）（2026-04-16実装完了）
...
6. **ゲストユーザー機能フェーズ2（バックエンドAPI）**（2026-04-16完了）
   - ゲスト予約作成・詳細取得・キャンセル・昇格API実装
   - メール通知機能（SES統合）
   - トークンベース認証システム

**現在のフェーズ**: フロントエンド開発（進行中）
- 会員/ゲストの区分を撤廃し、公開予約導線へ一本化済み（2026-06-21、コミット `7ae8b90`）
  - 誰でもログイン不要でカレンダー確認・予約が可能
  - 公開/埋め込みウィジェットは常にゲスト予約パス（`POST /reservations/guest`）を使用
  - admin の会員予約パスはバックエンド側に温存

**次のフェーズ**: バックエンド完全実装、E2Eテスト、オブザーバビリティ実装、デプロイ
...
| **公開予約導線 設計** | **[docs/guest-user-implementation.md](docs/guest-user-implementation.md)** | **会員/ゲスト区分を撤廃し公開予約導線へ一本化した現状の設計、バックエンド実装の位置づけ** |
...
- **不要なケース**: 単発の質問への回答や、明確に「次のアクション」が存在しない作業（純粋な調査タスクなど）では省略可
...
- **公開予約利用者**: ログイン不要でカレンダー閲覧・予約が可能な一般利用者。フロントでは会員/ゲストの区分を撤廃し公開予約導線に一本化（バックエンドには `customer`/`guest` ロールやゲスト予約API・トークン認証が実装基盤として残存）
