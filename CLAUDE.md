...
**ユーザー**:
- **スタジオ利用者 (customer)**: スタジオを予約・利用する顧客（会員）
- **ゲストユーザー (guest)**: ユーザー登録なしで予約可能な一時利用者（2026-04-16実装完了）
- **スタジオ管理者 (admin)**: 予約の承認・管理を行うスタジオ運営者
- **スタジオスタッフ (staff)**: 予約の閲覧のみ可能なスタッフ

**主要機能**:
- 予約カレンダー表示・予約作成（本予約/仮予約/ロケハン/第2キープ）
- **ゲスト予約機能**（ユーザー登録不要、トークンベース認証）（2026-04-16実装完了）
...
- ゲストユーザー機能フェーズ2（フロントエンド）実装予定

**次のフェーズ**: バックエンド完全実装、ゲストユーザーフェーズ3、E2Eテスト、オブザーバビリティ実装、デプロイ
