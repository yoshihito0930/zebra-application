...
func getGuestReservationHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	// パスパラメータからトークンを取得
	...
	// トークンで予約を取得
	reservation, err := reservationUsecase.FindByGuestToken(ctx, guestToken)
	if err != nil {
		switch err {
		case apierror.ErrReservationNotFound:
			return response.ErrorWithCORS(apierror.ErrReservationNotFound), nil
		default:
			log.Printf("Failed to find guest reservation: %v", err)
...
func handler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	// ゲスト予約取得は認証不要（トークンベース認証）
	return getGuestReservationHandler(ctx, request)
}
